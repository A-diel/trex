...
function draw(){
  ...
  if (keyDown("j")&&trex.y>=100){
    trex.velocityY=-12;       //{salto del trex
    saltoS.play();
  }
   trex.velocityY=trex.velocityY+2; //esta instucion es para la gravedad 
  ...
  if (gameState===play){
   ...
   if (suelo.x<0){
     suelo.x=suelo.width/2; //suelo infinito
      if (keyDown("j")&&trex.y>=100){
      trex.velocityY=-20;       //{salto del trex
      }
      trex.velocityY=trex.velocityY+10; //esta instucion es para la gravedad 
    }
    text("puntacion:"+puntuacion,500,50);
    puntuacion=puntuacion+Math.round(getFrameRate()/60);  //math.round(numeros enteros)
    nubes();
    obstaculos();
